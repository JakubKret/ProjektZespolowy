...
import 'package:krwiodawstwo/core/database/daos/menstrual_cycles_dao.dart';
import 'package:krwiodawstwo/core/database/tables/menstrual_cycles.dart';
...
void main() {
  ...
  group('createCycleAndRecalculate', () {
    ...
    test('deactivates the previously active cycle for the same donor',
      ...
      expect(rows[1].isActive, isTrue, reason: 'newest cycle should be active');
    });
    ...
    test('does not affect cycles of other donors', () async {
      ...
      final donor1Active = await (db.select(db.menstrualCyclesTable)
            ..where(
                (t) => t.donorProfileId.equals(1) & t.isActive.equals(true)))
          .get();
      final donor2Active = await (db.select(db.menstrualCyclesTable)
            ..where(
                (t) => t.donorProfileId.equals(2) & t.isActive.equals(true)))
          .get();
