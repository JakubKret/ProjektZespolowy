...
import 'package:hdk_mobile_app/core/database/daos/blood_centers_dao.dart';
import 'package:hdk_mobile_app/core/database/daos/donor_badges_earned_dao.dart';
import 'package:hdk_mobile_app/core/database/daos/donor_profile_dao.dart';
import 'package:hdk_mobile_app/core/database/daos/donations_dao.dart';
import 'package:hdk_mobile_app/core/database/daos/notification_log_dao.dart';
import 'package:hdk_mobile_app/core/database/daos/zhdk_badge_definitions_dao.dart';
...
void main() {
  ...
  group('DonationsDao', () {
    test('creates and fetches donor donations in descending date order', () async {
  ...
  group('NotificationLogDao', () {
    test('returns only pending due notifications', () async {
      ...
      await notificationLogDao.markSent(sentId, sentAt: DateTime(2026, 5, 1, 10, 5));
      ...
      final pending = await notificationLogDao.getPendingNotifications(asOf: now);
    ...
    test('markSent sets sentAt and dismiss sets isDismissed', () async {
      ...
      final row = (await notificationLogDao.getNotificationsForDonor(donorId)).single;
